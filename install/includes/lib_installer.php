 ...
 * @return  boolean     成功返回true，失败返回false
 */
function create_config_file($db_host, $db_port, $db_user, $db_pass, $db_name, $prefix, $timezone, $file_host, $dog_host, $local_host, $openstack_host,$system_manager_host)
{
    global $err, $_LANG;
    $db_host = construct_db_host($db_host, $db_port);
    ...
    $content .= "\$session_type    = \"mysql\";\n\n";
    $content .= "\$timezone    = \"$timezone\";\n\n";
    $content .= "\$cookie_path    = \"/\";\n\n";
    $content .= "\$cookie_domain    = \"\";\n\n";
    $content .= "\$session = \"1440\";\n\n";
    $content .= "\$base_url_config = array(\n\t'file_server' => \"$file_host\",\n\t'cloud_dog' => \"$dog_host\",\n\t'static_server' => \"\",\n\t'proxy_vnc' => \"\",\n\t'system_manager_host' => \"$system_manager_host\",\n\t'myself' => \"$local_host\"\n);\n\n";
    $content .= "\$openstack_proxy_host = false;\n\n";
    $content .= "\$openstack_proxy_host = \"\";\n\n";
    $content .= "\$openstack_identity = \"$openstack_host\";\n\n";
    $content .= "// openstack用户名\n";
    $content .= "\$openstack_username = \"\";\n\n";
    $content .= "// openstack密码\n";
    $content .= "\$openstack_password = \"\";\n\n";
    $content .= "// openstack用户名\n";
    $content .= "\$openstack_tenantName = \"\";\n\n";
    $content .= "// openstack云主机类型1-1-40的id\n";
    $content .= "\$openstack_flavorId = \"\";\n\n";
    $content .= "// 密钥对名称\n";
    $content .= "\$keypairName = \"\";\n\n";
    $content .= "\$region = \"\";\n\n";
    $content .= "\$urltype = \"\";\n\n";
    $content .= "\$is_openstack_huawei = false;\n\n";
    $content .= "\$huawei_openstack_network_id = \"\";\n\n";
    ...
    $content .= "define('ENVIRONMENT', 'production');\n\n";
    $content .= "define('DEBUG_MODE', 2);\n\n";
    $content .= '?>';

    /* 判断填写的路径是否正确 */

    if (trim($file_host) == '') {
        $err->add($_LANG['file_host_empty_error']);
        return false;
    }
    if (trim($dog_host) == '') {
        $err->add($_LANG['dog_host_empty_error']);
        return false;
    }
    if (trim($local_host) == '') {
        $err->add($_LANG['local_host_empty_error']);
        return false;
    }
    if (trim($openstack_host) == '') {
        $err->add($_LANG['openstack_host_empty_error']);
        return false;
    }
...
function create_admin_passport($admin_name, $admin_password, $admin_password2, $admin_email)
{
    ...
    if ($admin_name === '')
    {
        $err->add($_LANG['admin_name_empty_error']);
        return false;
    }  
