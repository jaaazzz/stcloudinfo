...
class Server extends NovaResource implements HasPtrRecordsInterface
{
    ...
    private function getAllocFloatingIPJson($pool)
    {
        $object = (object) array('pool' => $pool);
    ...
    private function getAddFloatingIPJson($ip)
    {
        $object = (object) array(
            'addFloatingIp'     => (object) array('address' => $ip)
        );
    ...
    private function getFloatingIP($pool)
    {
        $response = $this->getClient()->get($this->getParent()->getUrl('os-floating-ips'),self::getJsonHeader())->send();
        $body = Formatter::decode($response);

        //find the unbind ips in pool
        $free_floating_ips = array();
        foreach($body->floating_ips as $floating_ip)
        {
            if($floating_ip->instance_id == null && strcmp($floating_ip->pool,$pool) == 0)
            {
                $free_floating_ips[] = array('ip' => $floating_ip->ip,'id' => $floating_ip->id);
            }
        }

        if(count($free_floating_ips) == 0)
        {
            //alloc one
            $response = $this->getClient()->post($this->getParent()->getUrl('os-floating-ips'),self::getJsonHeader(),$this->getAllocFloatingIPJson($pool))->send();
            $body = Formatter::decode($response);
            return $body->floating_ip->ip;
        }
        else
        {
            $floating_ip = $free_floating_ips[0];
            return $floating_ip['ip'];
        }
    }

    public function bindFloatingIP($pool = 'nova')
    {
        $ip = $this->getFloatingIP($pool);
        $url = $this->url('action');
        $this->getClient()->post($url, self::getJsonHeader(), $this->getAddFloatingIPJson($ip))->send();
        return $ip;
    }

    public function getHypervisorsList()
    {
        $response = $this->getClient()->get($this->getParent()->getUrl('os-hypervisors/statistics'),self::getJsonHeader())->send();
        $body = Formatter::decode($response);
        return $body->hypervisor_statistics;
    }
}
