...
function admin_log($sn = '', $action, $content)
{
    ...
    $admin_id = $_SESSION['admin_id'] ? 0 : $_SESSION['admin_id'];
    $sql = 'INSERT INTO ' . $GLOBALS['ecs']->table('admin_log') . ' (log_time, user_id, log_info, ip_address) ' .
            " VALUES ('" . gmtime() . "', '$admin_id', '" . stripslashes($log_info) . "', '" . real_ip() . "')";
 ...
 */
function get_gs_goods_list($filter)
{
    $filter->keyword = json_str_iconv($filter->keyword);
    $where = get_where_sql($filter); // 取得过滤条件
    /* 取得数据 */
    $sql = 'SELECT goods_id, goods_name, shop_price, cat_id '.
        'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' . $where . '';
    /* Tom 这里去掉了limit。后台插件列表显示不完整 */
    $row = $GLOBALS['db']->getAll($sql);

    foreach($row as $key=>$val)
    {
        $cat = $GLOBALS['gis']->get_nick_name($val['cat_id']);
        $row[$key]['cat'] = $cat;
        if(substr($cat,1,1) != 'c') continue;

        $row[$key]['result'] = get_sub_goods_list($val['goods_id']);
    }
    return $row;
}

//获取框架的插件列表
function get_sub_goods_list($parent_id)
{
    $sql = 'SELECT goods_id, goods_name, shop_price '.
        'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS a where a.goods_id in (select goods_id from '
        .$GLOBALS['ecs']->table('group_goods'). ' as b where b.parent_id =' .$parent_id. ') and a.is_on_sale = 1 and a.is_delete = 0';
    /* Tom 这里去掉了limit。后台插件列表显示不完整 */
    //       'LIMIT 50';
...
function get_where_sql($filter)
{
    ...
    $where .= isset($filter->is_on_sale) && $filter->is_on_sale != '' ? " AND is_on_sale = '" . $filter->is_on_sale . "'" : '';
