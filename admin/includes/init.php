...
// if($source_type == "out"&&ENVIRONMENT!='development')
// {
//     die("not allowed.");
// }
...
// require(ROOT_PATH . 'includes/lib_domain.php');
...
$sess = new cls_session('ECSCP_ID',$redis_server,$redis_port);*/

/*require(ROOT_PATH . 'includes/cls_session_memcache.php');
$sess = new cls_session('ECSCP_ID',$memcache_server,$memcache_port);*/
...
$smarty->assign('file_server_base_url', $file_server_base_url);
...
if(isset($_SESSION['admin_id']) && intval($_SESSION['admin_id']) > 0){
    ...
    if($admin_id){
        ...
        if($_SESSION['admin_name']=='admin'){
            $_SESSION['role_list']='用户管理|user_list.php|用户管理.png,应用管理|app_manage.php|应用管理.png,资源管理|rm_cloudhost.php|资源管理.png,日志管理|admin_oper_log.php|日志管理.png,系统设置|set_config.php|系统设置.png';
        }
    }
    //设置管理员能看到的模块
    if($admin_id){
        $array1=explode(',',$_SESSION['role_list']);
        $role_array=array();
        foreach($array1 as $item){
            $array2=explode('|',$item);
            $role_array[]=array('value'=>$array2[0],'url'=>$array2[1],'img'=>$array2[2]);
        }
        $smarty -> assign('role_list',$role_array);
    }
    //设置门户标题
    $shop_title=$db->getOne("select value from ecs_shop_config where code= 'shop_title'");
    ...
    $_REQUEST['act'] != 'forget_pwd' && $_REQUEST['act'] != 'reset_pwd' && $_REQUEST['act'] != 'check_order')
{
    ...
    //echo $admin_path.preg_replace('/:\d+/', '', $_SERVER['HTTP_REFERER']);
...
require_once('../includes/lib_gisstore_init.php');
require_once(ROOT_PATH . 'admin/core/core.php');
