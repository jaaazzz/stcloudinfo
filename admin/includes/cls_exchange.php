...
class exchange
{
    ...
    var $name;
    var $error_msg;
    //一次对三个版本表进行处理　yukang 2015-1-27 $type=1
    var $type;
     ...
     * @param   int          $type        是否对三个版本的产品表进行同时处理(0:不处理1:处理)          
     *
     * @return void
     * @modify by huangbin 2016-2-02 为$type参数赋初始值0,解决首页提示缺少参数警告
     */
    function exchange($table, &$db , $id, $name,$type = 0)
    {
        $this->table     = $table;
        $this->db        = &$db;
        $this->id        = $id;
    ...
    {
        //一次对三个版本表进行处理　yukang 2015-1-27
        if($this->type==1 && $this->table=="`gs_store`.`ecs_goods`")
        {
            $sql1 = 'UPDATE `gs_store`.`ecs_beta_goods` SET ' . $set . " WHERE $this->id = '$id'";
            $this->db->query($sql1);

            $sql2 = 'UPDATE `gs_store`.`ecs_formal_goods` SET ' . $set . " WHERE $this->id = '$id'";
            $this->db->query($sql2);
        }  
