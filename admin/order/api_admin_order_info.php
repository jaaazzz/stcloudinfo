<?php 	
    ...
    if (isset($_REQUEST['order_id']))
    {
        ...
        $order = $GLOBALS['db']->getRow("
        	select ass.* ,user.user_name
        	from {$GLOBALS['ecs']->table('account_statement')} ass
        	left join {$GLOBALS['ecs']->table('users')} user on user.user_id = ass.user_id
        	where ass.account_statement_id = '$order_id'");
    }  
    else
    {
        /* 如果参数不存在，退出 */
    ...
    $sql = "SELECT MAX(order_id) FROM " . $ecs->table('account_statement') . " as o WHERE order_id < '$order_id'";
    $smarty->assign('prev_id', $db->getOne($sql));

    $sql = "SELECT MIN(order_id) FROM " . $ecs->table('account_statement') . " as o WHERE order_id > '$order_id'";   
    $smarty->assign('next_id', $db->getOne($sql));    

    /* invoice */
    $user_id = $order['user_id'] ;
    require_once(ROOT_PATH . "includes/lib_invoice.php");
    $invoice_info    = get_invoice_by_uid($user_id);
    $vatinvoice_info = get_vatinvoice_by_uid($user_id);
    $invoice_address = get_invoice_address_by_uid($user_id);
    $smarty->assign('invoice_info', $invoice_info);
    $smarty->assign('vatinvoice', $vatinvoice_info);
    $smarty->assign('invoice_address', $invoice_address);

    /* goods_list */
    require_once(ROOT_PATH . "includes/lib_bill.php");
    $order_info = processor_dispatcher($order);
    ...
    /* 取得订单操作记录 */
    $act_list = array();
    $sql = "SELECT * FROM " . $ecs->table('order_action') . " WHERE order_id = '$order[account_statement_id]' ORDER BY log_time DESC,action_id DESC";
    $res = $db->query($sql);
    while ($row = $db->fetchRow($res))
    {
        $row['order_status']    = $_LANG['cs'][$row['order_status']];
    ...
    else
    {
        ...
        /* 显示模板 */
        assign_query_info();
        $smarty->display('gis_order_info.htm');
    }