...
class integrate
{
     ...
     * @return int
     */
    function add_user($username, $password, $email, $gender = -1, $bday = 0, $reg_date=0, $md5password='',$uc_id = 0)
    {
        /* 将用户添加到整合方 */
        ...
        //wenbaolin 2014.11.18 add
        if($uc_id)
        {
            $fields = array($this->field_name, $this->field_email, $this->field_pass,'uc_id');
            $values = array($post_username, $email, $post_password,$uc_id);
        }
        else
        {
    ...
    function set_cookie($username='', $remember= null )
    {
        if (empty($username))
        {
            /* 摧毁cookie */
            $time = time() - 3600;
            setcookie("g_user_id",  '', $time, $this->cookie_path);            
            setcookie("g_password", '', $time, $this->cookie_path);
        ...
        elseif ($remember)
        {
            ...
            setcookie("gs_username", $username, $time, $this->cookie_path, $this->cookie_domain);
            $sql = "SELECT user_id, password FROM " . $GLOBALS['ecs']->table('users') . " WHERE user_name='$username' LIMIT 1";
            $row = $GLOBALS['db']->getRow($sql);
            if ($row)
            {
                //wenbaolin 2014.05.05 modify 给保存在cookie中的秘密重新加盐(另外一个生成盐的地方includes/init.php，两处生成的盐必须一致)
                $salt = empty($_SERVER['HTTP_USER_AGENT']) ? '~!d@#2%^&?*]|([/{;:}' : $_SERVER['HTTP_USER_AGENT'];
                $pwd = md5($salt . $row['password'] . "123");

                setcookie("g_user_id", $row['user_id'], $time, $this->cookie_path, $this->cookie_domain);
                setcookie("g_password", $pwd, $time, $this->cookie_path, $this->cookie_domain);
            }
        }
    }
