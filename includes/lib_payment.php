 ...
 * @return string
 */
function return_service_url($code){
    return $GLOBALS['ecs']->url() . 'service_respond.php?code=' .$code;
}

function check_pay_money($log_id, $money, $service_type){
    if(is_numeric($log_id))
    {
        if($service_type == "2"){
            $sql = 'SELECT training_fee FROM ' . $GLOBALS['ecs']->table('service_course_account').
...
function check_money($log_id, $money)
{
    if(is_numeric($log_id))
    {
        //wenbaolin 2015.01.21 因为历史原因，pay_log表中没有想要的金额数据，现在修改为读取account_statement表
        //$sql = 'SELECT order_amount FROM ' . $GLOBALS['ecs']->table('pay_log') .
        //      " WHERE log_id = '$log_id'";
        $sql = 'SELECT price FROM ' . $GLOBALS['ecs']->table('account_statement').
                " WHERE order_sn = '$log_id'";
        $amount = $GLOBALS['db']->getOne($sql);
    }
    else
