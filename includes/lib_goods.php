...
/**
 * Tom modified 2013-6-4 15:31:54 增加当前激活中的cat字段active
 * @param  integer $tree_id    [description]
 * @param  integer $cur_cat_id [description]
 * @return [type]              [description]
 */
function get_child_tree($tree_id = 0,$cur_cat_id = -1)
{
    $three_arr = array();
    ...
    if ($GLOBALS['db']->getOne($sql) || $tree_id == 0)
    {
        ...
        foreach ($res AS $row)
        {
            if ($row['is_show']){
                ...
                $three_arr[$row['cat_id']]['active']  = $GLOBALS['gis'] -> is_desc($cur_cat_id, $row['cat_id']);

                if (isset($row['cat_id']) != NULL)
                {
                    $three_arr[$row['cat_id']]['cat_id'] = get_child_tree($row['cat_id'],$cur_cat_id);
                }
            }
        }
    }
...
function get_recommend_goods($type = '', $cats = '')
{
    ...
    if (empty($type_goods[$type]))
    {
        ...
        foreach ($result AS $idx => $row)
        {
            ...
            $goods[$idx]['score']           = $row['goods_grade'];
            ...
            $goods[$idx]['thumb']        = convert_url_in_string(get_image_path($row['goods_id'], $row['goods_thumb'], true));
            $goods[$idx]['goods_img']    = convert_url_in_string(get_image_path($row['goods_id'], $row['goods_img']));
            $goods[$idx]['url']          = build_uri('goods', array('gid' => $row['goods_id']), $row['goods_name']);
            //$goods[$idx]['url']          = build_uri('detail', array('gid' => $row['goods_id']), $row['goods_name']);
...
function get_goods_by_id($request_goods_id)
{
    $goods_info = $GLOBALS['db']->getRow( "
        SELECT *
        FROM {$GLOBALS['ecs']->table( 'goods' )}
        WHERE goods_id = '$request_goods_id'"
    );
    return $goods_info;
}
//专为api　goods接口用通过产品ｉｄ查找产品详情
function get_api_goods_by_id($request_goods_id)
{
    $goods_info = $GLOBALS['db']->getRow( "
        SELECT *
        FROM {$GLOBALS['ecs']->table_org( 'goods' )}
        WHERE goods_id = '$request_goods_id'"
    );
    return $goods_info;
}
...
/**
 * 获得商品的详细信息
 * Tom 2013-7-18 16:20:27
 * 增加了获得定制或独立价格时，取得插件价格总和的逻辑
 * @access  public
...
function get_goods_info($goods_id)
{
    ...
    if ($row !== false)
    {
        ...
        $row['last_update']  = local_date($GLOBALS['_CFG']['date_format'], $row['last_update']);
        ...
        /* 修正商品图片 */
        $row['goods_img']   = zd_common_class::_convert_url_in_string(get_image_path($goods_id, $row['original_img']));
        $row['goods_thumb'] = zd_common_class::_convert_url_in_string(get_image_path($goods_id, $row['original_img'], true));
        $row['goods_desc']  = zd_common_class::_convert_url_in_string(stripcslashes($row['goods_desc']));
        $row['capacity']    = intval($row['file_size']);
        $row['has_file']    = $GLOBALS['gis']->has_file($row);

        $row['platform'] = $GLOBALS['gis']->get_platform($row['cat_id']);

        $developer_name = get_user_by_ucid($row['developer_id'],'user_name');
        $row['developer_name'] = $developer_name;// ? $developer_name : '未知';
...
/**
 * 获得商品的属性和规格
 * Tom Modified 2013-6-6 15:05:14
 * 增加合并规格属性的功能
 * @access  public
 * @param   integer $goods_id
 * @return  array
 */
function get_goods_properties($goods_id,$isJoinSpecificProperties = false)
{
    /* 对属性进行重新排序和分组 */
    ...
    $last_update = $GLOBALS['db'] -> getOne(
        "select last_update from "
        . $GLOBALS["ecs"] -> table("goods")
        . " where goods_id = '$goods_id';"
    );

    /* form last_update property, set attr_id = 0 */
    $res[] = [
        "attr_id" => "0",
        "attr_name" => "最后更新",
        "attr_value" => local_date("Y-m-d", $last_update)
    ];
    ...
    foreach ($res AS $row)
    {
        ...
        if($row['attr_name'] == '大小')
        {
            $row['attr_value'] = intval($row['attr_value']);
        }

        if ($row['attr_type'] == 0 || $isJoinSpecificProperties)
        {
            $group = (isset($groups[$row['attr_group']])) ? $groups[$row['attr_group']] : $GLOBALS['_LANG']['goods_attr'];

            $arr['pro'][$group][$row['attr_id']]['name']  = $row['attr_name'];

            //Tom added 2013-6-6 15:11:25
            $value = $arr['pro'][$group][$row['attr_id']]['value'];
            $arr['pro'][$group][$row['attr_id']]['value'] .= (isset($value) ? "," : "") . $row['attr_value'];
        }
        else
        {
 ...
 *
 * @access  public
 * @param $show_plugin 是否需要插件
 * @return  array
 */
function get_all_goods_info($show_plugin){
    $cat_id_array = get_all_packages_cat_id();
    if ($show_plugin) {
        $cat_sql = "";
    }
    else{
        $cat_sql = " AND gg.cat_id ".db_create_in($cat_id_array);
    }
    $sql = "SELECT gg.goods_id, gg.goods_name, g.cat_name, gg.cat_id" .
    " FROM " . $GLOBALS['ecs']->table('goods'). " AS gg " .
    " LEFT JOIN " . $GLOBALS['ecs']->table('category') . "AS g ON g.cat_id = gg.cat_id " .
    " WHERE gg.is_delete = 0 AND gg.is_on_sale = 1".$cat_sql;
    $result = $GLOBALS['db']->getAll($sql);
    return $result;
}
/**
 ...
 * @return  string
 */
function get_name_by_goods_id($goods_id){
...
/**
 * 获取框架（可定制）,框架（不可定制）cat_id
 *
 * @access  public
 * @return  array
 */
function get_all_packages_cat_id(){
    $cat_ids = [];
    foreach ($GLOBALS['gis'] -> all_types as $k => $v) {
        if($k[1] == "c" || $k[1] == "i")
        {
            $cat_ids[] = $v;
        }
...
function get_all_group_info(){
    $sql = "SELECT id,group_name,price,fun_desc" .
 ...
 * @return  array
 */
function get_group_plugin_by_id($goods_id){
 ...
 */
function get_group_id_by_addon_list($addon_arr){
    $group_id_arr = array();
    //需要返回的功能组id初始化
    $r_group_id = 0;
 ...
 * @return  bool
 */
function get_p_plugin_info_by_gid($goods_id){
    //根据$goods_id获取数据sql
    $get_info_sql = "SELECT id" .
                    " FROM " . $GLOBALS['ecs']->table('platform_plugin') .
                    " WHERE goods_id = '$goods_id'";
    //获取与否标志
    $flag = $GLOBALS['db']->getOne($get_info_sql);
    return $flag;
}
