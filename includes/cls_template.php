...
class cls_template
{
    ...
    /**
     * 处理缓存编号（根据内外网来存储不同的缓存）
     * @param $cache_id
     * wenbaolin 2014.05.30
     */
    function tran_cache_id(&$cache_id){

        global $source_type;

        $net_type = '_' . strtoupper($source_type);

        if($cache_id && strpos($cache_id,$net_type) === false){

            $cache_id = $cache_id . $net_type;
        }
    }
...
//            $source = preg_replace('/<head>/i', "<head>\r\n<meta name=\"Generator\" content=\"" . APPNAME .' ' . VERSION . "\" />",  $source);
            $source = preg_replace('/<head>/i', "<head>\r\n<meta http-equiv=\"X-UA-Compatible\" content=\"IE=edge\" />",  $source);
