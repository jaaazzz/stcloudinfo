...
function insert_member_info()
{
    ...
    $api_url = $ip."ecs/get/tenants";

    $message_result = zd_common_class::_send_get($api_url);
    
    $tenants = json_decode($message_result,true)['data'];

    $GLOBALS['smarty']->assign('tenants', $tenants);
    ...
    $GLOBALS['smarty']->assign('cloud_disk_url',$GLOBALS['cloud_disk_base_url']);
...
function insert_service_body_right(){
    $need_cache = $GLOBALS['smarty']->caching;
    $GLOBALS['smarty']->caching = false;
    if ($_SESSION['user_id'] > 0)
    {
        $GLOBALS['smarty']->assign('has_login', 'true');
    }
    else{
        $GLOBALS['smarty']->assign('has_login', 'false');
    }

    $output = $GLOBALS['smarty']->fetch('library/servicecenter/service_body_right.lbi');
...
//初始化行业列表,得到所有的行业，行业parent为8的
function insert_cat_init(){
    $sql="select cat_id as id,cat_name as name from {$GLOBALS['ecs']->table('category')} WHERE parent_id=117 order by sort_order";

    $cat_info = $GLOBALS['db']->getAll($sql);

    $GLOBALS['smarty'] -> assign('service_cat_info', $cat_info) ;
    //$cat_id = isset($_REQUEST['cat_id']) ? intval($_REQUEST['cat_id']) : 0;

    //$GLOBALS['smarty'] -> assign('service_cat_id', $cat_id) ;

    $output = $GLOBALS['smarty']->fetch('library/servicecenter/service_filter.lbi');

    return $output;
}

function insert_goods_init()
{
    $need_cache = $GLOBALS['smarty']->caching;
    $GLOBALS['smarty']->caching = false;
    ...
    if ($_SESSION['user_id'] > 0)
    {
        //$GLOBALS['smarty']->assign('has', mt_rand());
    }

    $goods_id = intval($_REQUEST['id']);

    //modify by zc 2015-12-22
    //改为调用通用方法，便于后期维护
    //$check_has_tried = check_has_trial($_SESSION[user_id],$goods_id);
//    $check_has_tried = $GLOBALS['db'] -> getOne("
//        SELECT count(orderinfo.order_id)
//        FROM {$GLOBALS['ecs']->table('order_goods')} ordergoods , {$GLOBALS['ecs']->table('order_info')} orderinfo
//        WHERE ordergoods.parent_id = 0 and orderinfo.is_trial = 1 and orderinfo.user_id = '{$_SESSION[user_id]}' and
//        orderinfo.order_id = ordergoods.order_id and ordergoods.goods_id = '$goods_id' ");
    //请求地址
    $ip = trim($GLOBALS['iggs_api_url_base_url']);

    $api_url = $ip."ecs/insert/GoodsInit";
    $api_url_s = $api_url."?userId=".$_SESSION[user_id]."&goodsId=".$goods_id;
    //发送请求
    $check_has_tried = zd_common_class::_send_get($api_url_s);
    $GLOBALS['smarty']->assign('can_try', $check_has_tried == '0');
    
    $output = $GLOBALS['smarty']->fetch('library/goods_init.lbi');
