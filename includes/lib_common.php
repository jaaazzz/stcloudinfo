...
function cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0, $is_show_all = true)
{
    ...
    if ($res === NULL)
    {
        ...
        if ($data === false)
        {
            $api_url = $GLOBALS['iggs_api_url_base_url'] . "ecs/categories";
            zd_core::autoload('zd_common_class');
            $json_result = zd_common_class::_send_get($api_url);
...
/*            $sql = "SELECT c.cat_id, c.cat_name, c.measure_unit, c.parent_id, c.is_show, c.show_in_nav, c.grade, c.sort_order, COUNT(s.cat_id) AS has_children ".
    ...
    if ($re_type == true)
    {
        ...
        {
            /**
             * Tom 2013-7-15 10:51:38
             * 增加类别的别名
             * 增加类别的全名
             */
            $cat_nick_name = $GLOBALS['gis']->get_nick_name($var['cat_id']);

            $select .= '<option nick_name="' . $cat_nick_name . '" value="' . $var['cat_id'] . '" ';
...
function load_config()
{
    ...
    if ($data === false)
    {
        $api_url = $GLOBALS['iggs_api_url_base_url']."ecs/shop/config";
        zd_core::autoload('zd_common_class');
        //发送请求
        $json_result = zd_common_class::_send_get($api_url);
        //数据数组
        ...
        //$res = $GLOBALS['db']->getAll($sql);
        if($result["code"] == 1 && is_array($result["data"])){
...
function order_action($order_sn, $order_status, $shipping_status="", $pay_status="", $note = '', $username = null, $place = 0)
{
    ...
    $sql = 'INSERT INTO ' . $GLOBALS['ecs']->table('order_action') .
            ...
            'SELECT ' .
                "account_statement_id, '$username', '$order_status', '$shipping_status', '$pay_status', '$place', '$note', '" .gmtime() . "' " .
            'FROM ' . $GLOBALS['ecs']->table('account_statement') . " WHERE order_sn = '$order_sn'";
 ...
 * @return  string
 */
function price_format($price, $change_price = true,$is_unit = true)
{
    if($price==='')
    {
     $price=0;
...
function build_uri($app, $params, $append = '', $page = 0, $keywords = '', $size = 0)
{
    ...
    switch ($app)
    {
        ...
        case 'detail':
            ...
            {
                $uri = $rewrite ? 'detail-' . $gid : 'detail.php?id=' . $gid;
            }
...
function get_image_path($goods_id, $image='', $thumb=false, $call='goods', $del=false)
{
    ...
    /**
     * Tom added 2013-6-28 15:16:47
     * 所有图片都取原图
     */
    //$url = preg_replace("//\/\/|\/goods_i\//", replacement, subject)
    //return $url;
    return $image;
}
 ...
 * @return string guid
 */
function gen_guid($is_md5 = false)
{
    $charid = strtoupper(md5(uniqid(mt_rand(), true))); 

    $hyphen = $is_md5 ? '' : chr(45);// "-" 
    ...
    return $is_md5 ? strtolower($uuid) : $uuid; 
}

function check_guid_format($str)
{
    return preg_match('/^[a-f0-9]{8}\-[a-f0-9]{4}\-[a-f0-9]{4}\-[a-f0-9]{4}\-[a-f0-9]{12}$/i', $str);
}   


function return_result($success,$msg="",$result = null,$url='')
{
    return array(
        "success" => !! $success,
        "msg"     => $msg,
...
/*输出xml或者json*/
function die_result($success,$msg="",$result = null,$url='')
{
    die(json_encode(return_result($success,$msg,$result,$url)));
}


function gr($arg_name,$reg = null)
{
    if($reg)
    {
        if(preg_match($reg, $_REQUEST[$arg_name]))
        {
            return mysql_real_escape_string($_REQUEST[$arg_name]);
        }
        else
        {
            return "";
...
function page_not_found(){
    ...
    show_message("404:您所访问的页面未找到",'','','error',false);
    exit;
}
...
function save_to_file($fileName,$XMLString)
{
...
function die_json($obj)
{
    die(json_encode($obj));
}

function get_user_by_id($user_id,$attr = '')
{
    ...
    $user_info['bank_name'] = get_bank_name($user_info['bank_type']);

    return $attr 
        ? ( isset($user_info[$attr]) ? $user_info[$attr] : false ) 
        : $user_info;
}

function get_user_by_ucid($user_id,$attr = '')
{
    $user_info = $GLOBALS['db']->getRow("
        SELECT *
        FROM {$GLOBALS['ecs']->table('users')}
...
function get_zd_key(){
    ...
    if (defined('ZD_ECS')){
        ...
        $out_str = 'cP6TwbT0PTxPeBcPTd11e11TPTP1k0o00ortyuim';
    }
...
/**
 * wenbaolin 2014.05.16
 *  此加密解密函数为对称加密算法，从Discuz的Ucenter移植过来的
 ...
 * @return string
 */
function zd_authcode($string, $operation = 'DECODE', $key = '', $expiry = 0) {
 ...
 * "sm"         手机短信
 */
function user_operate_log_add($content,$subject = "unbind")
{
    $operate_log = array(
        'ip'            => real_ip(),
    ...
    $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('user_operate_log'), $operate_log, 'INSERT');

}

function user_sm_log_count()
{
    $start_time = strtotime(date("Y-m-d 0:0:0", time()));
    $end_time = strtotime(date("Y-m-d 23:59:59", time()));

    $where_time = " log_time >= $start_time  AND log_time <=$end_time";

    $sql = 'SELECT count(id) as c FROM ' . $GLOBALS['ecs']->table('user_operate_log') .
        " WHERE subject = 'sm' AND user_id = " . $_SESSION['user_id'] ." AND " . $where_time;
 ...
 * @return [integer] [产品使用开始时间]
 *
 * huangbin 2015.11.27 modify 
 */
function get_order_start_time($start_time)
{
    $result_time = 0;
    $add_time = local_date('Y-m-d 00:00:00',$start_time);
 ...
 * huangbin 2015.11.27 modify 
 */
 function judge_is_inner_user($user_id){
    $user_info = $GLOBALS['db']->getRow("
        SELECT *
        FROM {$GLOBALS['ecs']->table('users')} 
        WHERE user_id= '$user_id'
    ...
    return preg_match("/@(mapgis)\./",$user_email);
 }
//判断是否内网IP
