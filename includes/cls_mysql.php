...
class cls_mysql
{
    ...
    {
        if(isset($GLOBALS['gis'] -> query_count))
        {
            $GLOBALS['gis'] -> query_count += 1;
        }
    ...
    function ErrorMsg($message = '', $sql = '')
    {
        ...
        /* 添加判断条件,工程在生成模式下关闭sql错误输出,添加错误文件 */
        else if (ENVIRONMENT == 'production') {
            //错误信息
            $write_message = $message ? $message : $this->error_message;
            error_log_write($write_message);
        }
        exit;
    }

/* 仿真 Adodb 函数 */
    //if $num <= 0, then there would be no limit
    ...
    function insert($table_name,$array){
        $sql='insert into '.$table_name.'('.implode(',',array_keys($array)).')';
