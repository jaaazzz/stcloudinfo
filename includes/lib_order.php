...
{
    $sql = 'SELECT pay_id, pay_name,pay_desc ' .
...
function get_payment_info_by_account_statement_id($id)
{
    return $GLOBALS['db'] -> getRow(
        "SELECT payment.* FROM "
        . $GLOBALS['ecs'] -> table('account_statement') . " account, "
        . $GLOBALS['ecs'] -> table('payment') . " payment "
        . " WHERE account.account_statement_id = '$id'"
        . " AND account.pay_id = payment.pay_id"
    );
}
...
{    
    return $GLOBALS['db']->getRow("select * 
        from {$GLOBALS['ecs']->table('account_statement')} ass where account_statement_id = '$order_id'");
}
...
{
    return $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('account_statement'),
        $order, 'UPDATE', "account_statement_id = '$order_id'");
}
...
/**
 * 针对gisstore编写的添加到购物车方法
 * 当添加的商品为定制的插件时，判断当前的购物车里的商品是否可以组合，如果不，返回相应的错误类型
 * 当添加的商品为独立的工具/应用时，清空当前的购物车，添加传来的商品信息；
 * 
 * TODO:添加的商品为打包好的插件集时，需要联合group表查询粗来插件列表，批量插入到购物车中去；group表仅当此项用处，别无他用；
 * TODO:添加的商品为插件时，查询该插件是否已经在其他该用户购买过的商品中被购买，如果是，这个价格不被计算在总价内。
 * TODO:目前除了增加插件，进行其他操作前都会清空购物车！
 * @param   integer $goods_id   商品编号
 * @param   integer $num        商品数量
 * @param   array   $spec       规格值对应的id数组
 * @param   integer $parent     基本件
 * @return  boolean
 */
function gisstore_addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0)
{   
    $sql = "select * from " . $GLOBALS['ecs'] -> table('goods') . " where goods_id = $goods_id ";
    $cur_goods = $GLOBALS['db'] -> getRow($sql);
    $goods_cat_id = $cur_goods['cat_id'];

    if(!isset($goods_cat_id)){ return false; }
    
    /*检查商品类型*/
    $gis_type = $GLOBALS['gis'] -> get_nick_name($goods_cat_id);

    /*类型为打包好的工具时*/
    if($gis_type == 'dc' || $gis_type == 'wc' ){
        /*暂时清空购物车！*/
        clear_cart();
        $parentId = -1;$sum_market_price = 0;$sum_shop_price = 0;
        $sql = 'select a.goods_id,goods_sn,goods_name,market_price,shop_price from '
            . $GLOBALS['ecs'] -> table('group_goods') . "a," 
            . $GLOBALS['ecs'] -> table('goods') 
            . "b where a.parent_id = $goods_id and b.goods_id = a.goods_id";
        $row = $GLOBALS['db'] -> getAll($sql);
        ...
        foreach ($row as $goods) {
            ...
            $sum_market_price += $goods['market_price'];
            $sum_shop_price += $goods['shop_price'];
            $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('cart'), $temp_good, 'INSERT');
        }
        ...
        $parent = array(
            'user_id'       => $_SESSION['user_id'],
            'session_id'    => SESS_ID,
            'goods_id'      => $cur_goods['goods_id'],
            'goods_sn'      => $cur_goods['goods_sn'],
            'goods_name'    => $cur_goods['goods_name'],
            'market_price'  => $sum_market_price,
            'goods_price'   => $sum_shop_price,
            'goods_number'  => 1
        ); 
    ...
    if(substr($gis_type, 1 , 1) == 'i'){
        ...
        $goods = array(
            ...
            'goods_sn'      => $cur_goods['goods_sn'],
            'goods_name'    => $cur_goods['goods_name'],
            'market_price'  => $cur_goods['market_price'],
            'goods_price'   => $cur_goods['shop_price'],
            'goods_number'  => 1
        ); 

        $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('cart'), $goods, 'INSERT');
    }   

    /**
     * 类型为插件时，第一个插件进入购物车时增加一个id为-1的父商品用于记录名称和总价
     * TODO：checkout的时候再计算总价格    
     */
    if(substr($gis_type, 1, 1) == 'c' && strlen($gis_type) == 3){
        $sql = "select g.cat_id,c.* from " . $GLOBALS['ecs'] -> table('cart') . " c,".$GLOBALS['ecs'] -> table('goods') 
            . " g where c.goods_id = g.goods_id and session_id = '" . SESS_ID . "'";
        $goods_in_cart = $GLOBALS['db'] -> getAll($sql);
        $goods_cat_array = array($cur_goods['cat_id']);
        ...
        foreach ($goods_in_cart as $goods) {
            if($goods['cat_id'] !== -1){
                 ...
                 * 如果购物车里已经存在，那么直接返回
                 */
                if($goods['goods_id'] == $cur_goods['goods_id']){
                    return true;
                }else{
                    $goods_cat_array[] = $goods['cat_id'];
                }
            }
        }

        $insert_new_record = $GLOBALS['gis'] -> can_be_combined($goods_cat_array);

        /*如果不匹配，返回错误*/
        if(!$insert_new_record){
            $GLOBALS['err']->add(sprintf($GLOBALS['_LANG']['wrong_addon_type'], $num), ERR_CART_ADDON_TYPE);
            return false;
        }

        /*如果购物车还是空的，那么增加parent商品*/
        if(count($goods_in_cart) == 0){
            $parent = array(
                'user_id'       => $_SESSION['user_id'],
                'session_id'    => SESS_ID,
                'goods_id'      => -1,
                'goods_sn'      => 'C00000',
                'goods_name'    => $GLOBALS['gis'] -> custom_product_name,
                'market_price'  => 0,
                'goods_price'   => 0,
                'goods_number'  => 1
            ); 
        ...
        $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('cart'), $goods_tobe_insert, 'INSERT');
...
/**
 * Tom 2013-7-10 10:56:01
 * 改版后的添加到购物车
 * 用户定制或独立工具（独立工具addon不传）
 * @param  int      $parent_id   父亲的商品id
 * @param  array    $addon_array 下属插件的id
 * @param  string    $package_name 定制的名称
 * @return boolean               成功与否
 */
function gis_add_to_cart($parent_id,$addon_array = array(),$package_name, $start_time, $end_time)
{   
    $sql = "select * from " . $GLOBALS['ecs'] -> table('goods') . " where goods_id = $parent_id ";
    $cur_goods = $GLOBALS['db'] -> getRow($sql);
    $goods_cat_id = $cur_goods['cat_id'];

    if(!$goods_cat_id){ return false; }
    ...
    if(strlen($gis_type) != 3){ return false; }

    /*类型为打包好的工具时*/
    if(substr($gis_type, 1 , 1) == 'c'){       
        if(!count($addon_array)){ die(json_encode(array('success' => false,'msg' => '你还没有选择插件！'))); }
        $sum_market_price = 0;$sum_shop_price = 0;
        $sql = 'select a.goods_id,goods_sn,goods_name,market_price,shop_price from '
            . $GLOBALS['ecs'] -> table('group_goods') . "a," 
            . $GLOBALS['ecs'] -> table('goods') 
            . "b where a.parent_id = $parent_id and b.goods_id = a.goods_id";
        $row = $GLOBALS['db'] -> getAll($sql);

        /*逐个插入插件*/
        foreach ($row as $goods) {
            foreach ($addon_array as $item) {                 
                if(intval($item['goods_id']) == intval($goods['goods_id']) && intval($goods['goods_id'])) {
                    $temp_good = array(
                        'user_id'       => $_SESSION['user_id'],
                        'session_id'    => SESS_ID,
                        'goods_id'      => $goods['goods_id'],
        ...
        $parent = array(
            ...
            'goods_id'      => $parent_id,
            'goods_sn'      => $cur_goods['goods_sn'],
            'goods_name'    => $package_name ? mysql_real_escape_string($package_name) : $cur_goods['goods_name'],
            'market_price'  => $sum_market_price,
            'goods_price'   => $sum_shop_price,
            'goods_number'  => 1,
            'start_time'    => $start_time,
 ...
 * @param integer $pay_account 要修改的价格
 * @return bool
 */
function update_account_statement_pay_account($order_sn,$pay_account){

    if(empty($order_sn) || empty($pay_account)){
        return false;
    }

    $account_statement = $GLOBALS['ecs']->table('account_statement');
    $sql = "update " . $account_statement . " SET pay_account ='$pay_account' " .
...
function array_formater(&$result_all_array)
{
    if( !$result_all_array ){ return; }

    $last_ele = &$result_all_array[count($result_all_array) - 1];

    $last_ele['size'] = $last_ele['size'] == 0 
        ? '未知' 
        : get_real_size($last_ele['size']);

    $temp_obj = count($last_ele['addon_list']) > 0 
        ? $last_ele['addon_list'][0] 
        : $last_ele;

    $last_ele['envs'] = get_goods_envs($last_ele['cat_id'],$last_ele['envs']);
...
function get_user_order($user_id){
    ...
    SELECT orderinfo.order_id,orderinfo.add_time,orderinfo.end_time,orderinfo.order_sn,orderinfo.order_count,
        orderinfo.is_trial, orderinfo.is_delete,
        goods.goods_name,goods.version,goods.original_img,goods.file_size,goods.is_on_sale,
        goods.cat_id,goods.envs,goods.goods_id,goods.shop_price,renew.start_time,
        ordergoods.goods_name as goods_new_name,ordergoods.parent_id,goods.file_info,goods.goods_desc
    FROM {$GLOBALS['ecs']->table('goods')} goods,
        {$GLOBALS['ecs']->table('order_goods')} ordergoods,
        {$GLOBALS['ecs']->table('order_info')} orderinfo
    LEFT join (
        SELECT MAX( end_time ) , MAX( start_time ) as start_time , user_id, order_id
        FROM  {$GLOBALS['ecs']->table(renew_trial)} 
        GROUP BY user_id, order_id
        ) renew on (orderinfo.order_id = renew.order_id and renew.user_id = '$user_id')
    WHERE orderinfo.user_id = $user_id and orderinfo.order_id = ordergoods.order_id and              
        orderinfo.order_status = 1 and orderinfo.pay_status = 2 and 
        goods.goods_id = ordergoods.goods_id
    ORDER by orderinfo.add_time desc,orderinfo.order_id asc,ordergoods.parent_id asc,goods.goods_id asc         
    ");
    ...
    {
        if($item['order_id'] != $temp_order_id)
        {
            $nick_name = $GLOBALS['gis']->get_nick_name($item['cat_id']);

            $cat_info = $GLOBALS['gis']->get_cat_info($item['cat_id']);

            if(count($result_all_array) > 0)
            {
               array_formater($result_all_array);
            }            
            ...
            $result_all_array[] =  array(
                ...
                'goods_img'       => convert_url_in_string($item['original_img']),
                'order_id'        => $item['order_id'],
                'thumb'           => convert_url_in_string($item['original_img']),
                ...
                'date'            => local_date('Y-m-d 00:00',$item['end_time']),
                // 'date'            => date('Y-m-d H:i',$item['end_time']),
                'create_time'     => local_date('Y-m-d H:i',$item['add_time']),            
                'rest_time'       => $rest_time,
                'size'            => 0,
                'top_cat'         => $nick_name[0],
                'type'            => $nick_name[1],
                //'can_renew_trial' => $now_time > intval($item['start_time']),
                'size'            => intval($item['file_size']),  
                'price'           => $goods_total_price ? $goods_total_price : $item['shop_price'],
                'addon_list'      => array(),
                'envs'            => $item['envs'],
        ...
        else
        {
            ...
            $addon_item = array(
                ...
                'final_price' => $goods_total_price ? $goods_total_price : $item['shop_price'],
                'envs'   => $item['envs']
            );

            $last_ele['size'] += intval($item['file_size']);
            array_push($last_ele['addon_list'], $addon_item);
        }
    }
    array_formater($result_all_array);
 ...
 * @return user_id 
 */
function get_user_by_order_sn($order_sn){
    //订单购买用户
    $account_user = $GLOBALS['db']->getOne("
        SELECT user_id 
