...
function setting_ui_api(result)
{
  if(result)
  {
    ...
    // f["js-file-host"].onblur = function  () {
    //   var file_host = f['js-file-host'].value;
    //   if (!checkUrlEmpty(file_host)) {
    //     $("js-install-at-once").setAttribute("disabled", "true");
    //     $("js-file-host-result").innerHTML="<span class='comment'><img src='images\/no.gif'>"+$_LANG["file_empty_error"]+"<\/span>";
    //   }else if (!checkUrl(file_host)) {
    //     $("js-install-at-once").setAttribute("disabled", "true");
    //     $("js-file-host-result").innerHTML="<span class='comment'><img src='images\/no.gif'>"+$_LANG["address_error"]+"<\/span>";
    //   }else {
    //     $("js-file-host-result").innerHTML="<img src='images\/yes.gif'>";
    //     if (checkAllUrl()) {
    //       $("js-install-at-once").removeAttribute("disabled");
    //     }
    //   }
...
function checkAllUrl(){
  var file_host = document.getElementsByName('js-file-host')[0].value;
  var dog_host = document.getElementsByName('js-dog-host')[0].value;
  var local_host = document.getElementsByName('js-local-host')[0].value;
  var openstack_host = document.getElementsByName('js-openstack-host')[0].value;
  return checkUrlEmpty(file_host) && checkUrlEmpty(dog_host) && checkUrlEmpty(local_host) && checkUrlEmpty(openstack_host) && checkUrl(file_host) && checkUrl(dog_host) && checkUrl(local_host) && checkUrl(openstack_host);
}
function checkUrlEmpty(urlStr){
