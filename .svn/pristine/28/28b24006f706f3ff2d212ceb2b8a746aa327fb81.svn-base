...
function addMapService(mapType, mapName, token, mapSvrPort) {
    ...
    switch (mapType) {
        ...
        case "docWms":
            ...
            //基于地图文档的WMS访问基地址
            var baseUrlDocWms = "http://" + data.svrCfg.svrIp + ":" + mapSvrPort + "/igs/rest/ogc/doc/" + mapName + "/WMSServer";
        ...
        case "docWfs":
            var getWfsDocOgcCapabilitiesUrl = "http://" + data.svrCfg.svrIp + ":" + mapSvrPort + "/igs/rest/ogc/doc/" + mapName + "/WFSServer?service=WFS&REQUEST=GetCapabilities&version=1.0.0";
            getCapabilities(getWfsDocOgcCapabilitiesUrl, getOgcWfsMapInfoSuccess);

            var baseUrlDocWfs = "http://" + data.svrCfg.svrIp + ":" + mapSvrPort + "/igs/rest/ogc/doc/" + mapName + "/WFSServer";
        ...
        case "TILE_OGC":
            var getWmtsOgcCapabilitiesUrl = "http://" + data.svrCfg.svrIp + ":" + mapSvrPort + "/igs/rest/ogc/WMTSServer?service=WMTS&REQUEST=GetCapabilities&version=1.0.0";
            getCapabilities(getWmtsOgcCapabilitiesUrl, getOgcWmtsMapInfoSuccess);

            //基于矢量图层的WMS访问基地址
            var baseUrlWmts = "http://" + data.svrCfg.svrIp + ":" + mapSvrPort + "/igs/rest/ogc/WMTSServer";
...
function loadMap(globe, mapName) {
    //设置三维地图服务信息
    var ip = data.svrCfg.svrIp;           //地图服务的IP地址
