...
function get_main_sender($email)
{
    $email_corp = explode('@', $email)[1];

    if($email_corp == 'zondycyber.com')
    {
        return $zondy_smtp_setting;
    }
    else
    {
        return array(
            ...
            'smtp_pass' => $GLOBALS['_CFG']['smtp_pass']
        );
    }
}
...
function send_mail($name, $email, $subject, $content, $type = 0, $notification=false)
{
    ...
    else
    {
        ...
        /* 获得邮件服务器的参数设置 */
        $smtp_setting = get_main_sender($email);

        $params['host'] = $smtp_setting['smtp_host'];
        $params['port'] = $smtp_setting['smtp_port'];
        $params['user'] = $smtp_setting['smtp_user'];
        $params['pass'] = $smtp_setting['smtp_pass'];
 ...
 * @author      huangbin
 * @access      public
 * @param       string $arg  错误信息
 * @param       string $file 错误文件
 * @param       string $line 错误行数
 *
 * @return      bool
 */
function error_log_write($arg, $file = '', $line = '')
{
    //存放错误文件夹路径
    $folder_path = ROOT_PATH . DATA_DIR . '/log';
    //创建文件夹,如果已存在,则判定是否存在此路径
    if (make_dir($folder_path)) {
        //以当前时间命名文件
        $file_source = $folder_path.'/'.date('Y-m-d').'-'.date('H-i-s').'.txt';
        //错误信息初始化
        $str = "FILE: $file\r\nLINE: $line\r\n";
