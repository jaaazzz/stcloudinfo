 ...
 * @return  void
 */
function handle_group_goods($goods_id, $is_plugin = false)
{
...
function build_attr_html($cat_id, $goods_id = 0)
{
    ...
    foreach ($attr AS $key => $val)
    {
        ...
        if ($val['attr_input_type'] == 0)
        {
            ...
            if($val['attr_name'] == "大小" || $val['attr_name'] == "版本"){
                $html .= " readonly='readonly' style='background-color:#EEE;border:1px solid #AAA;height:18px;width:100px;' ";
            }
            $html .= ' /> ';
            if($val['attr_name'] == "大小") { $html .= "单位：B"; }
        }
 ...
 * @return  array
 */
function get_group_goods($goods_id, $is_from_package_table = false)
{
    $sql = "SELECT gg.goods_id, gg.parent_id, CONCAT(g.goods_name, ' -- [', gg.goods_price, ']',' ',if(is_basic=0,'可选','必选')) AS goods_name " .
            "FROM " . $GLOBALS['ecs']->table('group_goods') . " AS gg, " .
                $GLOBALS['ecs']->table('goods') . " AS g ";
    if($is_from_package_table)
    {
        $sql .= " WHERE gg.goods_id = '$goods_id'"
            . " AND gg.parent_id = g.goods_id";
    }
    else
    {
        $sql .= " WHERE gg.parent_id = '$goods_id'" 
...
function goods_list($is_delete, $real_goods=1, $conditions = '')
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT goods_id, is_has_beta, is_has_formal, goods_name, goods_type, goods_sn, shop_price, is_on_sale, is_best, is_new, is_hot, is_official,sort_order, goods_number, integral, " .
...
function service_goods_list()
{
    ...
    {
        $where .= " AND goods_type = '".$filter['goods_type']."' ";
    }   
    /* 记录总数 */
    $sql = "SELECT COUNT(*) FROM " .$GLOBALS['ecs']->table('service_goods'). " AS g WHERE is_delete = 0 $where";
    $filter['record_count'] = $GLOBALS['db']->getOne($sql);
    ...
    /* 获取数据 */
    $sql = "SELECT * FROM " .$GLOBALS['ecs']->table('service_goods'). " AS g WHERE is_delete = 0 $where" .
           " ORDER BY $filter[sort_by] $filter[sort_order] " .
           " LIMIT " . $filter['start'] . ",$filter[page_size]";
