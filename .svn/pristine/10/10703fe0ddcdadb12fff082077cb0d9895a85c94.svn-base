 ...
 * @param   int          $uid               ucenter uid
 * @param   array        $other             注册的其他信息
 *
 * @return  bool         $bool
 */
function register($username, $password, $email, $uid, $other = array(), $need_pwd = false)
{
    /* 检查注册是否关闭 */
    ...
    /* WARNING!!!! Try not to modify error_no, once its modified, you need to go to api/v1/lib/lib_error_msg.php 
        and modify function translate_error_no() and inform developer of dev_center */

    /* 检查email */
    if (empty($email))
    {
        $GLOBALS['err']->add($GLOBALS['_LANG']['email_empty'], 4);
    }
    else
    {
        if (!is_email($email))
        {
            $GLOBALS['err']->add(sprintf($GLOBALS['_LANG']['email_invalid'], htmlspecialchars($email)), 4);
        }
    }
    ...
    {
        $GLOBALS['err']->add(sprintf($GLOBALS['_LANG']['username_exist'], $username), 3);
        return false;
    }

    if (strlen($password) < 6 && $need_pwd)
    {
        $GLOBALS['err']->add("length of password must be no less than 6", 7);
        return false;
    }

    if (!$GLOBALS['user']->add_user($username, $password, $email, $gender = -1, $bday = 0, $reg_date=0, $md5password='',$uid))
    {
        if ($GLOBALS['user']->error == ERR_INVALID_USERNAME)
        {
            $GLOBALS['err']->add(sprintf($GLOBALS['_LANG']['username_invalid'], $username), 1);
        }
        elseif ($GLOBALS['user']->error == ERR_USERNAME_NOT_ALLOW)
        {
            $GLOBALS['err']->add(sprintf($GLOBALS['_LANG']['username_not_allow'], $username), 2);
        }
        ...
        {
            $GLOBALS['err']->add(sprintf($GLOBALS['_LANG']['email_invalid'], $email), 4);
        }
        elseif ($GLOBALS['user']->error == ERR_EMAIL_NOT_ALLOW)
        {
            $GLOBALS['err']->add(sprintf($GLOBALS['_LANG']['email_not_allow'], $email), 5);
        }
        elseif ($GLOBALS['user']->error == ERR_EMAIL_EXISTS)
        {
            $GLOBALS['err']->add(sprintf($GLOBALS['_LANG']['email_exist'], $email), 6);
        }
        else
        {
    ...
    else
    {
        ...
        if ($other['is_developer'])
        {
            $GLOBALS['db'] -> query(
                "UPDATE " . $GLOBALS['ecs'] -> table('users')
                . " SET is_developer = '{$other['is_developer']}'"
 ...
 * @return  boolean
*/
function user_registered($user_name){
