    ...
    if ($res)
    {
        while ($row = $GLOBALS['db']->fetchRow($res))
        {
            $temp = [
                'id'          => $row['article_id'],
                'title'       => $row['title'],
                'short_title' => $GLOBALS['_CFG']['article_title_length'] > 0 ? sub_str($row['title'], $GLOBALS['_CFG']['article_title_length']) : $row['title'],
                'author'      => empty($row['author']) || $row['author'] == '_SHOPHELP' ? $GLOBALS['_CFG']['shop_name'] : $row['author'],
                'url'         => $row['open_type'] != 1 ? build_uri('article', array('aid'=>$article_id), $row['title']) : trim($row['file_url']),
                'add_time'    => date($GLOBALS['_CFG']['date_format'], $row['add_time']),
                'content'     => $row['content']
            ];
            $arr[] = $temp;
        }
...
function get_article_cat($parent_id)
{
    return $GLOBALS['db']->getAll(
        'SELECT * FROM ' . $GLOBALS['ecs']->table('article_cat') .
        " WHERE parent_id = '$parent_id' and show_in_nav = 1".
        ' ORDER BY  cat_id ASC'
    );
}
