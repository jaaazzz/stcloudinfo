               ...
               success: function(result) {
                    ...
                    for(var i=0;i<result.length;i++){
                        if(result[i].lx=="1"){
                            ...
                            data.xCenter=result[i].centerx;
                            data.yCenter=result[i].centery;
                            data.zoom=result[i].zoom;
                            zoompoi = result[i].zoom;
                            centerpoi = [result[i].centerx,result[i].centery];
                            projection=result[i].projection;
                            // alert(result[i].layer);
...
function serchClick(option) {
    ...
    if (wherecon != "") {
        ...
        if($.trim(option.data.searchVal) != "" || selectedClass != ""){
            //IE浏览器会自动把“01”转换成“1”导致查询结果出问题，所以这里做一下处理
            ...
            if(selectedClass ==""){
                ...
                selectedClass = wherecon;
            }else{
                //如果点击大类生成结果集之后，再次根据搜索框执行搜索（关键字回车进行查询），
                //则在结果集中进行筛选
                queryParam.where = "CLASID LIKE '" + selectedClass + "%' AND NAME LIKE '%" + $("#sole-input").val() + "%'";
            }
        }
        else{
