...
olMap.changeMapZIndex=function(layer){
	if(map!=null){
		var layers=map.getLayerGroup().getLayers().getArray();
	    for(var i=0;i<layers.length;i++){
...
basicTree.hideLayer=function(mc){
	...
	if(mc){
		...
		for(var i=0;i<layers.length;i++){
			if(layers[i].isdefault!=true){
				layers[i].setVisible(false);
			}
	    }
	}
}
basicTree.addLayer=function(pro,maptype,lx,service,url,mc,zoom,centerx,centery,node,tree,theme){
	var projection=olMap.tranProjection(pro);
	var flag=0;
	var layers=map.getLayerGroup().getLayers().getArray();
	if(layers.length>0){
...
basicTree.defaultMap=function(msg){
	if(!msg.layer.isNullOrEmpty()){
		if(msg.maptype==1){//瓦片
			olMap.showMap(msg.projection,msg.text,msg.layer,msg.centerx,msg.centery,msg.zoom,msg.maptype,msg.lx);
		}else if(msg.maptype==2){//mapx文档
			olMap.showMap(msg.projection,msg.text,msg.layer,msg.centerx,msg.centery,msg.zoom,msg.maptype,msg.lx);
		}else if(msg.maptype==3){//PGIS
			olMap.showPGIS(msg.url,msg.text,msg.layer,msg.centerx,msg.centery,msg.zoom,msg.lx);
		}
	}
}
        ...
        onSelect: function (node) {
            ...
            if (node.type == 1) {
                if (node.checked == true) {
                    ...
                    basicTree.addLayer(node.projection,node.maptype,node.lx,node.layer,node.url,node.text,node.zoom,node.centerx,node.centery,node,$('#basictree'),1);
                }
            }
        },
