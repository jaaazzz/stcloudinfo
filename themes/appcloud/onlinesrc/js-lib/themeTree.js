		...
		onSelect: function (node) {
			...
			if(node.type == 1){
				...
				if(node.checked == true){
					...
					basicTree.addLayer(node.projection,node.maptype,node.lx,node.layer,node.url,node.text,node.zoom,node.centerx,node.centery,node,$('#themetree'));
				}
			}else if(node.type == 0){
				for(var i in nodes){
				...
				if(node.checked == true){
					...
					for(var i in children){
						if(children[i].type == 1){
							basicTree.addLayer(children[i].projection,children[i].maptype,children[i].lx,children[i].layer,children[i].url,children[i].text,children[i].zoom,children[i].centerx,children[i].centery,children[i],$('#themetree'));
						}
					}
				}
			}
