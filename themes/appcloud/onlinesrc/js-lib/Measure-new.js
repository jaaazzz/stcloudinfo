...
function fullscreen(){
    if($('#swipe').attr('show')==1||$('#multi').attr('show')==1){
        ...
        if(map1!=null){
            map1.setTarget();
            map1.setTarget($("#map1").get(0));
        }
        if(map2!=null){
            map2.setTarget();
            map2.setTarget($("#map2").get(0));
        }
    }else{
        $('#onlinesrc-content').css('height','100%');
        $('.navbar-wrapper').toggle();
        ...
        if($('.navbar-wrapper').css('display') == "none"){
           ...
           $('.zoom-control-mapCon').css({
                    background:"url(themes/appcloud/onlinesrc/images/左键收缩.png)50% 50% no-repeat"
            })
           zoomControlFlag = 1;
          // map.setTarget($("#mapCon").get(0));
        }
...
//切换图标
function changeblue() {
    src = 'themes/appcloud/onlinesrc/images/markers/markers_newp0.png';
    data.newFeature.setStyle(createLabelTextStyle(data.newFeature));
};
function changered() {
    src = 'themes/appcloud/onlinesrc/images/markers/markers_p0.png';
    data.newFeature.setStyle(createLabelTextStyle(data.newFeature));
};
